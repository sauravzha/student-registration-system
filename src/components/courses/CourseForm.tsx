...
const CourseForm: React.FC<CourseFormProps> = ({ editingId, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const existingNames = courses
      .filter(c => c.id !== editingId)
      .map(c => c.name);
    
    const validation = validateCourse(name, existingNames, editingId);
    ...
    if (editingId && editingCourse) {
      ...
      dispatch({ 
        type: 'SHOW_TOAST', 
        payload: { message: 'Course updated successfully', type: 'success' } 
      });
    } else {
      const newCourse = {
        id: generateId(),
      ...
      dispatch({ 
        type: 'SHOW_TOAST', 
        payload: { message: 'Course created successfully', type: 'success' } 
      });
    }
